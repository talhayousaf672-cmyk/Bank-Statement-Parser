"""Bank Alfalah statement parser.

Layout Assumptions:
- Input language: English
- Pakistani bank — pipe-delimited tabular layout
- Columns: Date | Narration | Ref No | Debit | Credit | Balance
- Dates formatted as DD/MM/YYYY
- IBAN shown in header
- Currency: PKR
"""
...
_ACCOUNT_RE = re.compile(r"(?:Account No|IBAN)[.:\s]+([A-Z0-9\-]+)", re.IGNORECASE)
_HOLDER_RE = re.compile(r"(?:Account Title|Name)[:\s]+(.+)", re.IGNORECASE)
_CURRENCY_RE = re.compile(r"Currency[:\s]+([A-Z]{3})", re.IGNORECASE)
_PERIOD_RE = re.compile(
    r"(?:From|Period From)[:\s]+(\d{2}/\d{2}/\d{4})\s+(?:To|Period To)[:\s]+(\d{2}/\d{2}/\d{4})",
    re.IGNORECASE,
)
...
class BankAlfalahParser(BaseBankParser):
    bank_id = "bank_alfalah"
    ...
    def parse(self, normalized_text: str) -> ParseResult:
        ...
        for line in normalized_text.splitlines():
            ...
            debit = _parse_decimal(parts[3] if len(parts) > 3 else "")
            credit = _parse_decimal(parts[4] if len(parts) > 4 else "")
            balance = _parse_decimal(parts[5] if len(parts) > 5 else "")

            amount = -debit if debit is not None else (credit if credit is not None else Decimal("0.00"))
            ...
            transactions.append(Transaction(
                transaction_date=tx_date, value_date=tx_date, description=description,
