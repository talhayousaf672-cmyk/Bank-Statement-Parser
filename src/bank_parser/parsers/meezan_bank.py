"""Meezan Bank statement parser.

Layout Assumptions:
- Input language: English
- Pakistani Islamic bank — pipe-delimited tabular layout
- Columns: Date | Narration | Cheque/Ref No | Debit | Credit | Balance
...
def _parse_dmy_slash(raw: str) -> date | None:
    raw = raw.strip()
    try:
        d, m, y = raw.split("/")
...
def _parse_dmy_mon(raw: str) -> date | None:
    raw = raw.strip()
    try:
        parts = raw.split("-")
        if len(parts) != 3:
...
class MeezanBankParser(BaseBankParser):
    bank_id = "meezan_bank"
    ...
    def parse(self, normalized_text: str) -> ParseResult:
        ...
        if pm:
            period_start = _parse_dmy_mon(pm.group(1))
            period_end = _parse_dmy_mon(pm.group(2))
        ...
        for line in normalized_text.splitlines():
            ...
            if parts[0].upper() in ("DATE", ""):
                continue
            tx_date = _parse_dmy_slash(parts[0])
            if tx_date is None:
                continue
    ...
    def _build_tx(
        ...
        debit_raw = parts[3] if len(parts) > 3 else ""
        credit_raw = parts[4] if len(parts) > 4 else ""
